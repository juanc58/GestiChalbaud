...
<body class="bg-gray-50 p-4 md:p-12">
    ...
    <!-- Print Control -->
    <div class="max-w-full space-y-6 no-print flex justify-between items-center mb-8 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        @php
            $backRoute = auth()->user()->isAdmin() 
                ? route('students.show', $enrollment->student_id) 
                : route('representative.students.index');
        @endphp
        <a href="{{ $backRoute }}" class="text-sm font-bold text-gray-500 hover:text-[#1A237E] transition-colors flex items-center">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            Volver
        </a>
        <button onclick="window.print()" class="bg-[#1A237E] text-white px-8 py-3 rounded-xl font-bold hover:bg-[#1A237E]/90 transition-all shadow-lg flex items-center">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
            Imprimir Boletín
        </button>
    </div>

    <div class="max-w-full space-y-6 bg-white p-12 rounded-xl shadow-xl border border-gray-100 print-container">
        <!-- Header -->
        <header class="flex justify-between items-start border-b-2 border-gray-100 pb-8 mb-8">
            <div class="flex items-center">
                <div class="w-16 h-16 bg-[#1A237E] rounded-2xl flex items-center justify-center text-white mr-4 shadow-lg">
                    <span class="font-black text-xl">SEP</span>
                </div>
                <div>
                    <h1 class="text-2xl font-black text-[#1A237E] uppercase tracking-tight">S.E.U.E.D PÁEZ</h1>
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mt-1">Ministerio del Poder Popular para la Educación</p>
                    <p class="text-[10px] font-bold text-[#FBC02D] uppercase tracking-[0.1em]">BOLETÍN INFORMATIVO DE DESEMPEÑO ACADÉMICO</p>
                </div>
            </div>
            <div class="text-right">
                <div class="inline-block bg-[#1A237E] px-4 py-2 rounded-xl text-white text-xs font-black uppercase tracking-widest">
        ...
        <section class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-10">
            <div>
                ...
                <p class="text-sm font-extrabold text-[#1A237E]">{{ $enrollment->student->first_name }} {{ $enrollment->student->last_name }}</p>
            </div>
            <div>
                <p class="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Cédula de Identidad</p>
                <p class="text-sm font-extrabold text-[#1A237E]">{{ $enrollment->student->cedula ?? 'S/C' }}</p>
            </div>
            <div>
                <p class="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Grado / Nivel</p>
                <p class="text-sm font-extrabold text-[#1A237E] uppercase">{{ $enrollment->section->grade->name }}</p>
            </div>
            <div>
                <p class="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Sección / Turno</p>
                <p class="text-sm font-extrabold text-[#1A237E] uppercase">"{{ $enrollment->section->name }}" ({{ $enrollment->section->shift }})</p>
            </div>
        </section>

        <!-- Grades Table -->
        <section class="mb-10">
            <h2 class="text-xs font-black text-[#FBC02D] uppercase tracking-[0.2em] mb-4 flex items-center">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach($subjects as $subject)
                            ...
                            <tr class="hover:bg-gray-50/30 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center font-black text-[#1A237E]">{{ $m1 ?? '-' }}</td>
                                <td class="px-6 py-4 text-center font-black text-[#1A237E]">{{ $m2 ?? '-' }}</td>
                                <td class="px-6 py-4 text-center font-black text-[#1A237E]">{{ $m3 ?? '-' }}</td>
                                <td class="px-6 py-4 text-center font-black text-[#FBC02D] bg-gray-50/50 text-base">
                                    {{ $m3 ?? ($m2 ?? ($m1 ?? '-')) }}
                                </td>
                            </tr>
                        @endforeach
        ...
        <section class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            <div class="space-y-6">
                <div>
                    <h3 class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2 flex items-center">
                        <span class="w-2 h-2 rounded-full bg-[#1A237E] mr-2"></span>
            ...
            <div class="space-y-6">
                <div>
                    <h3 class="text-[10px] font-black text-[#FBC02D] uppercase tracking-[0.2em] mb-2 flex items-center">
                ...
                <div class="pt-4">
                    <div class="p-6 rounded-2xl border-4 border-double {{ $enrollment->status == 'Promovido' ? 'border-green-200 bg-green-50/50' : 'border-blue-100 bg-blue-50' }} text-center">
                        ...
                        <p class="text-2xl font-black {{ $enrollment->status == 'Promovido' ? 'text-green-600' : 'text-[#1A237E]' }} uppercase italic">
                            {{ $enrollment->status }}
                        </p>
                    </div>
                </div>

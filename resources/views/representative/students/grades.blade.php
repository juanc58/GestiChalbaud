...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex justify-between items-center bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
        <div>
            <h3 class="text-2xl font-extrabold text-[#032e5e]">Historial Académico</h3>
        ...
        <div class="flex items-center gap-4">
            <form action="{{ route('representative.students.grades', $student->id) }}" method="GET" id="yearFilterForm" class="flex items-center gap-3">
                ...
                <select name="school_year" onchange="document.getElementById('yearFilterForm').submit()" 
                    class="px-4 py-2 bg-gray-50 border border-gray-100 rounded-xl font-bold text-[#032e5e] text-xs outline-none focus:border-blue-200 transition-all appearance-none cursor-pointer pr-10 relative">
    ...
    @if($enrollment)
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-8 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
                <div>
                    <span class="text-[10px] font-black text-[#c56c39] uppercase tracking-[0.2em]">{{ $enrollment->school_year }}</span>
                    <h4 class="text-xl font-extrabold text-[#032e5e]">{{ $enrollment->section->grade->name }}</h4>
                ...
                <div class="flex flex-col items-end gap-2">
                    ...
                    <a href="{{ route('reports.boletin', $enrollment->id) }}" target="_blank" class="px-4 py-2 bg-[#032e5e] text-white rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-[#032e5e]/90 transition-all flex items-center shadow-lg shadow-[#032e5e]/10">
            ...
            <div class="p-8">
                <div class="grid grid-cols-1 gap-8">
                    @forelse($enrollment->assessments->groupBy('subject_id') as $subjectId => $assessments)
                        ...
                        <div class="bg-gray-50/50 rounded-[2rem] border border-gray-100 overflow-hidden group hover:border-blue-100 transition-all">
                            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white group-hover:bg-blue-50/30 transition-all">
                                <div>
                                    <h5 class="font-extrabold text-[#032e5e] text-sm">{{ $subject->name }}</h5>
                            ...
                            <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                                @for($term = 1; $term <= 3; $term++)
                                    ...
                                    <div class="p-4 rounded-2xl {{ $assessment ? 'bg-white shadow-sm border-blue-50' : 'bg-gray-100/30 border-dashed border-gray-200' }} border-2 flex flex-col items-center group/moment hover:scale-105 transition-all">
                                        ...
                                        <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl font-black {{ $assessment ? 'text-[#c56c39] bg-orange-50/50' : 'text-gray-300' }}">
                                            {{ $assessment->score ?? '-' }}
                                        </div>
                                    </div>
                                @endfor

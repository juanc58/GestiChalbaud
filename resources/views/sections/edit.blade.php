...
<div class="max-w-2xl mx-auto">
    <div class="mb-8 flex items-center space-x-4">
        ...
        <div>
            <h3 class="text-2xl font-extrabold text-[#032e5e]">Editar Sección</h3>
            <p class="text-gray-500 font-semibold">Modifica los detalles del aula: {{ $section->grade->name }} "{{ $section->name }}"</p>
        </div>
    </div>

    <form action="{{ route('sections.update', $section->id) }}" method="POST" class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-6">
            <div class="space-y-2">
                ...
                <input type="text" name="name" placeholder="Ej: A, B, U" required maxlength="2" value="{{ old('name', $section->name) }}" class="w-full px-5 py-4 rounded-2xl bg-gray-50 border-2 {{ $errors->has('name') ? 'border-red-300' : 'border-transparent' }} focus:border-[#032e5e]/20 outline-none transition-all font-bold text-xl uppercase text-center text-[#032e5e]">
                @error('name')
        ...
        <div class="space-y-2 pt-4">
            <label class="text-xs font-black text-[#c56c39] uppercase ml-1 tracking-widest">Profesor Encargado (Año {{ $currentYear }})</label>
        ...
        <div class="pt-4 mt-6 border-t border-gray-100">
            <button type="submit" class="w-full py-4 rounded-2xl font-bold bg-[#c56c39] text-white hover:bg-[#c56c39]/90 transition-all shadow-xl shadow-[#c56c39]/20 text-lg">
                Actualizar Sección
            </button>
        </div>
    </form>

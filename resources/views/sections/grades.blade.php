...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    <div class="p-8 border-b border-gray-50 flex flex-col md:flex-row justify-between items-center bg-gray-50/50 gap-4">
        <div>
            <h3 class="text-2xl font-extrabold text-[#1A237E]">Carga de Calificaciones</h3>
            <p class="text-gray-500 font-semibold mt-1">Gestión académica: Aula "{{ $section->name }}" - {{ $section->grade->name }}</p>
        </div>
        <div class="flex items-center space-x-3">
            <span class="px-4 py-2 bg-white border border-gray-100 rounded-xl text-xs font-bold text-[#FBC02D] shadow-sm">
    ...
    <div class="p-6 border-b border-gray-50 bg-white">
        <form action="{{ route('sections.grades', $section->id) }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                ...
                <select name="subject_id" onchange="this.form.submit()" class="w-full px-5 py-3 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-[#1A237E]/10 outline-none transition-all font-bold text-gray-700 appearance-none">
            ...
            <div>
                ...
                <select name="term" onchange="this.form.submit()" class="w-full px-5 py-3 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-[#1A237E]/10 outline-none transition-all font-bold text-gray-700 appearance-none">
    ...
    <form action="{{ route('sections.grades.store', $section->id) }}" method="POST">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td class="px-6 py-5 text-center">
                            <select name="grades[{{ $enrollment->id }}][score]" 
                                class="w-20 px-3 py-3 rounded-xl bg-gray-50 border-2 border-transparent focus:border-[#1A237E]/20 focus:bg-white text-center font-bold text-gray-800 outline-none transition-all appearance-none cursor-pointer">
                                <option value="">--</option>
                        ...
                        <td class="px-6 py-5">
                            <input type="text" name="grades[{{ $enrollment->id }}][observations]" value="{{ old('grades.'.$enrollment->id.'.observations', $obs) }}" placeholder="Describe el desempeño..." 
                                class="w-full px-4 py-3 rounded-xl bg-gray-50 border-2 border-transparent focus:border-[#1A237E]/10 focus:bg-white text-sm font-semibold text-gray-600 outline-none transition-all">
                        </td>
                        <td class="px-6 py-5 text-center">
                            @if($assessment)
        ...
        <div class="p-8 bg-gray-50/50 border-t border-gray-50 flex justify-end">
            <button type="submit" class="bg-[#1A237E] text-white px-12 py-4 rounded-2xl font-extrabold text-sm hover:bg-[#1A237E]/90 transition-all shadow-xl shadow-[#1A237E]/20 flex items-center">

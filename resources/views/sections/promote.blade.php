...
<div class="max-w-6xl mx-auto space-y-8">
    ...
    <div class="flex justify-between items-end">
        <div>
            ...
            <h3 class="text-3xl font-extrabold text-[#1A237E]">Cierre de Año Escolar</h3>
            <p class="text-gray-500 font-semibold mt-1">
                Promoviendo desde: <span class="text-[#FBC02D]">{{ $section->grade->name }} "{{ $section->name }}"</span>
            </p>
        </div>
        <div class="bg-white px-8 py-5 rounded-xl shadow-sm border border-gray-100 text-right">
            <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none mb-1 text-center">Próximo Periodo</div>
            <div class="text-2xl font-black text-[#1A237E]">{{ $nextYear }}</div>
        </div>
    </div>

    <!-- Alert about Year Switching -->
    <div class="p-6 bg-blue-50/50 rounded-xl border border-blue-100/50 flex items-start space-x-4">
    ...
    <form action="{{ route('sections.promote.store', $section->id) }}" method="POST" id="promoteForm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 h-[650px]">
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col space-y-6">
                ...
                <div class="relative">
                    <input type="text" id="studentSearch" placeholder="🔍 Buscar por apellido o C.I..." class="w-full px-6 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-[#1A237E]/20 outline-none transition-all font-semibold">
                </div>
            ...
            <div class="bg-white p-8 rounded-xl shadow-sm border border-orange-100 flex flex-col space-y-6 relative overflow-hidden">
                <div class="absolute top-0 right-0 w-48 h-48 bg-orange-50/40 rounded-full -mr-24 -mt-24 z-0"></div>
                
                <div class="flex justify-between items-center relative z-10">
                    <h4 class="text-xs font-bold text-[#FBC02D] uppercase tracking-[0.2em]">Listos para Aplicar</h4>
                    <span id="readyCount" class="px-3 py-1 bg-[#FBC02D]/10 text-[#FBC02D] rounded-full text-[10px] font-black">0 PROCESADOS</span>
                </div>
                ...
                <div class="pt-4 relative z-10">
                    <button type="submit" id="submitBtn" disabled class="w-full py-5 rounded-xl font-bold bg-gray-100 text-gray-400 cursor-not-allowed transition-all shadow-xl text-lg flex items-center justify-center group active:scale-95">
...
<!-- Configuration Modal -->
<div id="configOverlay" class="fixed inset-0 bg-[#1A237E]/40 backdrop-blur-sm z-50 flex items-center justify-center opacity-0 pointer-events-none transition-all duration-300">
        ...
        <div class="p-10 space-y-8">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div id="statusPass" onclick="toggleStatus('Aprobado')" class="p-4 rounded-2xl border-2 border-[#1A237E] bg-blue-50/50 cursor-pointer transition-all flex flex-col items-center text-center">
                        ...
                        <span class="font-bold text-[#1A237E] text-sm italic">APROBADO</span>
                    </div>
            ...
            <div class="space-y-4">
                ...
                <select id="modalTargetSection" class="w-full px-6 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-[#1A237E]/20 outline-none transition-all font-bold text-[#1A237E] appearance-none">
            ...
            <button type="button" onclick="confirmStudentConfig()" class="w-full py-5 rounded-2xl font-bold bg-[#FBC02D] text-[#1A237E] hover:bg-[#FBC02D]/90 transition-all shadow-xl shadow-sm text-lg">
                Procesar Estudiante
            </button>
        </div>
    </div>
...
<script>
    ...
    function toggleStatus(status) {
        ...
        if (status === 'Aprobado') {
            passCard.className = "p-4 rounded-2xl border-2 border-[#1A237E] bg-blue-50/50 cursor-pointer transition-all flex flex-col items-center text-center";

...
<div class="space-y-8">
    <div class="flex justify-between items-end">
        <div>
            <div class="flex items-center space-x-3 mb-2">
                <a href="{{ route('sections.index') }}" class="text-gray-400 hover:text-[#1A237E] transition-colors font-bold text-sm flex items-center">
            ...
            <h3 class="text-3xl font-extrabold text-[#1A237E] flex items-center">
            ...
            <div class="flex items-center space-x-4 mt-2">
                ...
                <form action="{{ route('sections.show', $section->id) }}" method="GET" id="yearFilterForm">
                    <select name="school_year" onchange="document.getElementById('yearFilterForm').submit()" class="bg-white border-2 border-gray-100 rounded-xl px-4 py-1.5 font-bold text-[#1A237E] focus:border-[#1A237E]/20 outline-none transition-all shadow-sm">
        ...
        <div class="flex space-x-3">
            @if($isLatestYear || $isAdmin)
                ...
                <a href="{{ route('sections.grades', $section->id) }}" class="bg-[#1A237E] text-white px-6 py-3 rounded-xl font-bold hover:bg-[#1A237E]/90 transition-all shadow-sm flex items-center text-sm">
                ...
                <a href="{{ route('sections.promote.view', $section->id) }}" class="bg-[#FBC02D] text-[#1A237E] px-6 py-3 rounded-xl font-bold hover:bg-[#FBC02D]/90 transition-all shadow-lg shadow-sm flex items-center text-sm">
                ...
                @endif
                @if($isAdmin)
                    <a href="{{ route('sections.edit', $section->id) }}" class="p-3 bg-white text-gray-400 hover:text-[#FBC02D] rounded-xl border border-gray-100 shadow-sm transition-all" title="Editar detalles del aula">
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div>
                <h4 class="text-[10px] font-black text-[#FBC02D] uppercase tracking-widest mb-1">Docente Encargado</h4>
                @if($currentTeacher)
                    <p class="text-lg font-extrabold text-[#1A237E]">{{ $currentTeacher->first_name }} {{ $currentTeacher->last_name }}</p>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td class="px-8 py-5">
                            <div class="flex items-center">
                                <div class="w-8 h-8 rounded-full bg-gray-100 text-[#1A237E] font-bold text-xs flex items-center justify-center mr-3 border border-gray-200">
                        ...
                        <td class="px-8 py-5 text-right space-x-3 flex items-center justify-end">
                            <a href="{{ route('reports.boletin', $enrollment->id) }}" target="_blank" class="text-[#FBC02D] hover:text-[#FBC02D]/80 transition-all" title="Ver Boletín">
                            ...
                            <a href="{{ route('students.show', ['student' => $enrollment->student->id, 'redirect_section' => $section->id]) }}" class="text-gray-400 hover:text-[#1A237E] transition-colors font-bold text-xs uppercase tracking-widest">Ver Perfil</a>

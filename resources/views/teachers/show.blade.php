...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex justify-between items-end">
        <div>
            <h3 class="text-2xl font-extrabold text-[#032e5e]">Perfil del Docente</h3>
        ...
        <div class="flex space-x-3">
            ...
            <a href="{{ route('teachers.edit', $teacher->id) }}" class="px-6 py-2 rounded-xl bg-white text-[#c56c39] font-bold border border-orange-100 hover:bg-orange-50 transition-all text-sm">Editar Datos</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="space-y-6">
            <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col items-center">
                <div class="w-24 h-24 rounded-full bg-[#032e5e]/5 mb-4 flex items-center justify-center border-4 border-[#032e5e]/10">
                    <span class="text-3xl font-bold text-[#032e5e]">{{ substr($teacher->first_name, 0, 1) }}{{ substr($teacher->last_name, 0, 1) }}</span>
                </div>
                <h4 class="font-extrabold text-[#032e5e] text-center">{{ $teacher->first_name }} {{ $teacher->last_name }}</h4>
            ...
            <div class="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
                <h5 class="text-xs font-bold text-[#c56c39] uppercase tracking-widest mb-4">Información de Contacto</h5>
                <div class="space-y-4">
                    <div>
        ...
        <div class="md:col-span-2 space-y-6">
            ...
            <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
                ...
                @if($assignment)
                    <div class="p-6 rounded-2xl bg-[#c56c39]/5 border border-[#c56c39]/10 flex justify-between items-center">
                        <div>
                            <p class="text-[10px] font-black text-[#c56c39] uppercase tracking-widest mb-1">Aula Asignada</p>
                            <h6 class="text-xl font-black text-[#032e5e]">{{ $assignment->section->grade->name }} "{{ $assignment->section->name }}"</h6>
                        ...
                        <div class="flex flex-col items-end">
                            ...
                            <a href="{{ route('sections.show', $assignment->section_id) }}" class="text-xs font-bold text-[#032e5e] hover:underline">Ver Aula</a>
                        </div>
                    </div>
                @else
                    <div class="p-8 rounded-3xl border-2 border-dashed border-gray-100 text-center">
                        ...
                        <form action="{{ route('teachers.assign-section', $teacher->id) }}" method="POST" class="flex flex-col md:flex-row gap-4 justify-center items-end">
                            ...
                            <div class="text-left">
                                ...
                                <select name="section_id" required class="px-5 py-3 rounded-xl bg-gray-50 border-2 border-transparent focus:border-[#032e5e]/10 outline-none font-bold text-gray-700 text-sm appearance-none min-w-[200px]">
                            ...
                            <button type="submit" class="bg-[#032e5e] text-white px-8 py-3.5 rounded-xl font-black text-xs hover:bg-[#032e5e]/90 transition-all shadow-lg uppercase tracking-widest">
                                Asignar Ahora
                            </button>
                        </form>
                    </div>
                    ...
                    @forelse($teacher->assignments->where('school_year', '!=', $currentYear)->sortByDesc('school_year') as $history)
                        <div class="flex justify-between items-center p-4 rounded-xl border border-gray-50 bg-gray-50/30">
                            <div>
                                ...
                                <p class="text-sm font-bold text-[#032e5e]">{{ $history->section->grade->name }} "{{ $history->section->name }}"</p>
                            </div>

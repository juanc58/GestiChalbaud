...
@section('content')
<div class="mb-8">
    <h3 class="text-2xl font-extrabold text-[#032e5e]">Directorio de Representantes</h3>
    ...
    <div class="p-6 border-b border-gray-50 flex flex-col md:flex-row gap-4 bg-gray-50/50 items-center justify-between">
        <form action="{{ route('representatives.index') }}" method="GET" class="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
            ...
            <div class="flex gap-2">
                <button type="submit" class="bg-[#c56c39] text-white px-6 py-2 rounded-xl font-bold text-sm hover:bg-[#c56c39]/90 transition-all flex-1 shadow-md shadow-[#c56c39]/20">
                    Buscar
                </button>
                ...
                <tr class="hover:bg-orange-50/30 transition-colors">
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            <div class="w-10 h-10 rounded-full bg-orange-100 text-[#c56c39] flex items-center justify-center font-bold mr-3 border border-orange-200">
                                {{ substr($rep->first_name, 0, 1) }}{{ substr($rep->last_name, 0, 1) }}
                            </div>
                            <div class="flex flex-col">
                                <span class="font-bold text-[#032e5e]">{{ $rep->first_name }} {{ $rep->last_name }}</span>
                    ...
                    <td class="px-6 py-4 flex items-center space-x-3">
                        <a href="{{ route('representatives.edit', $rep->id) }}" class="p-2 text-[#c56c39] hover:text-[#032e5e] hover:bg-orange-50 rounded-xl transition-all" title="Ver y Editar Ficha">

...
<div class="max-w-full space-y-6 space-y-8">
    <div>
        <h3 class="text-2xl font-extrabold text-[#1A237E]">Configuración de Perfil</h3>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="md:col-span-2 space-y-6">
            <form action="{{ route('profile.update') }}" method="POST" class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 space-y-6">
                @csrf
                @method('PUT')

                <h4 class="text-xs font-black text-[#1A237E] uppercase tracking-widest mb-4">Datos de Identidad</h4>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="group space-y-2">
                        ...
                        <input type="text" name="first_name" value="{{ old('first_name', $user->first_name) }}" required
                            class="w-full px-5 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-[#1A237E]/10 focus:bg-white outline-none transition-all font-bold text-[#1A237E]">
                ...
                <div class="pt-4 border-t border-gray-100">
                    <h5 class="text-[10px] font-black text-[#1A237E] uppercase tracking-widest mb-4">Datos Profesionales</h5>
                    <div class="group space-y-2">
                ...
                <div class="pt-4 border-t border-gray-100 space-y-4">
                    <h5 class="text-[10px] font-black text-[#FBC02D] uppercase tracking-widest">Contacto Adicional</h5>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="group space-y-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="group space-y-2">
                            ...
                            <input type="text" name="job_title" value="{{ old('job_title', $user->representative->job_title) }}"
                                class="w-full px-5 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-green-100 focus:bg-white outline-none transition-all font-bold text-[#1A237E]">
                        </div>
                        <div class="group space-y-2">
                ...
                <div class="pt-4">
                    <button type="submit" class="bg-[#1A237E] text-white px-8 py-4 rounded-2xl font-bold hover:bg-[#1A237E]/90 transition-all shadow-sm text-sm">
                        Guardar Cambios
                    </button>
                </div>
            </form>
            ...
            <!-- Password Change -->
            <form action="{{ route('profile.password.update') }}" method="POST" class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    <div class="group space-y-2">
                        ...
                        <input type="password" name="current_password" required
                            class="w-full px-5 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-red-100 focus:bg-white outline-none transition-all font-bold text-[#1A237E]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="group space-y-2 md:col-span-2">
                        ...
                        <input type="text" name="security_answer" placeholder="Tu nueva respuesta secreta" autocomplete="off"
                            class="w-full px-5 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-amber-200 focus:bg-white outline-none transition-all font-bold text-[#1A237E]">
        ...
        <div class="space-y-6">
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col items-center text-center">
                <div class="w-20 h-20 rounded-full bg-orange-50 flex items-center justify-center text-[#FBC02D] mb-4 border-2 border-orange-100 text-3xl font-black">
                    {{ substr($user->first_name ?? '?', 0, 1) }}{{ substr($user->last_name ?? '', 0, 1) }}
                </div>
                <h5 class="font-extrabold text-[#1A237E]">{{ $user->full_name }}</h5>
                <span class="mt-2 px-4 py-1.5 bg-[#FBC02D]/10 text-[#FBC02D] rounded-full text-[10px] font-black uppercase tracking-widest border border-orange-100">
                    {{ $user->role->name }}
                </span>
